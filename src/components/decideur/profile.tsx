...
export default function Parametres() {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.put(`http://localhost:3001/profile/update/${user.id}`, formData);
  ...
  return (
    <div className="bg-white p-2 rounded-lg shadow">
      ...
      {isPopupVisible && (
        <div 
          className={`fixed bottom-4 left-1/2 transform -translate-x-1/2 p-3 rounded-lg shadow-md w-80 text-center ${popupType === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white`}
        >
          <div>
            {popupMessage}
          </div>
